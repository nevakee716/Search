...
(function (cwApi) {
  ...
  var engine, forbiddenProperties = ['cwaveragerating', 'cwtotalcomment', 'exportflag'],
  ...
  createDocument = function(properties, o){
    ...
    for(i=0; i<properties.length; i+=1){
      val = cwApi.cwPropertiesGroups.getDisplayValue(o.objectTypeScriptName, properties[i].scriptName, o.properties[properties[i].scriptName], o, 'properties');
  ...
  function removeDiacritics(str) { // remove special characters from string
  ...
  engine.prototype.prepareSearch = function(scope, opt){
    var i = 0, enableSearch, x, config, j, p, res = {};
  ...
  engine.prototype.searchAllItems = function(q, scope, opt){
    var terms = q.split(' '), i, j, t, prep, n, x, res = [], ref;
    prep = this.prepareSearch(scope, opt);
    for(n in prep){
      if (prep.hasOwnProperty(n)){
        x = prep[n].data;
